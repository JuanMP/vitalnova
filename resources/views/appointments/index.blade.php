                                ...
                                <div class="card-action">
                                    @if ($appointment->status_id == 1)
                                        <a href="{{ route('appointments.edit', $appointment->id) }}"
                                            class="btn blue waves-effect waves-light">Editar</a>
                                        <form action="{{ route('appointments.destroy', $appointment->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn red waves-effect waves-light">Eliminar</button>
                                        </form>
                                    ...
                                    @elseif($appointment->status_id == 3)
                                        ...
                                        {{-- <a href="{{ route('reviews.create', $appointment->id) }}" class="btn green waves-effect waves-light">Valorar</a> --}}
                                        <form action="{{ route('reviews.create', $appointment->id) }}" method="GET"
                                            style="display:inline;">
                                            @csrf
                                            <button type="submit"
                                                class="btn green waves-effect waves-light">Valorar</button>
                                        </form>
                            ...
                            <div class="card">
                                <div class="card-content">
                                    ...
                                    @if (Auth::user()->isDoctor())
                                        ...
                                        <p><strong>Observaciones: </strong> {{ $appointment->observations }}</p>
                                        @if ($appointment->status_id == 1)
                                            <form action="{{ route('appointments.start', $appointment->id) }}"
                                                method="POST" style="display:inline;">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn green waves-effect waves-light">Iniciar
                                                    Cita</button>
                                            </form>
                                        @elseif($appointment->status_id == 2)
                                            <form action="{{ route('appointments.finish', $appointment->id) }}"
                                                ...
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn orange waves-effect waves-light">Finalizar
                                                    Cita</button>
                                            </form>
                                        @elseif($appointment->status_id == 3)
